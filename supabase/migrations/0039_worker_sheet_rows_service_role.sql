-- The public/admin report RPC intentionally keeps the staff-role gate.
-- The isolated GitHub worker has no user session, so it needs a separate
-- service-role-only surface with the same official form projection.
create or replace function public.mother_sangha_form_rows_worker_v1(requested_year text, requested_level text)
returns table (
...
as $$
  ...
  left join activity_form_submissions a on a.academic_year_id = y.id and a.student_id = s.id
    and a.application_level = r.dhamma_level and a.exam_status = 'exam'
  left join lateral (
...
grant execute on function public.mother_sangha_form_rows_worker_v1(text, text) to service_role;
