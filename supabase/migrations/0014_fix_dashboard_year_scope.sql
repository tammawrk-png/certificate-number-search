-- Keep the public dashboard aggregate scoped to the academic year it reports.
-- The earlier successor counted every active student when more than one year
-- existed, which could overstate the current roster.
...
as $$
  select
    ...
    (select count(*)
      ...
      where s.status = 'active' and c.academic_year_id = y.id and c.active),
    (select count(distinct cm.student_id)
       from certificate_matches cm
       join students ms on ms.id = cm.student_id and ms.status = 'active'
       join classrooms mc on mc.id = ms.classroom_id and mc.academic_year_id = y.id and mc.active
      where cm.status in ('auto_matched', 'confirmed')),
...
comment on function public.public_dashboard_metrics_v2() is
  'Aggregate-only public dashboard data scoped to each academic year.';
