(() => {
  ...
  const recommendationFor = (row) => {
    ...
    // No matched certificate is a valid starting point for ชั้นตรี.
    // A missing historical number must not block a first-time applicant.
    if (!row.match_status || row.match_status === 'unmatched') return { level:'ตรี', basis:'ยังไม่พบประวัติเดิม · สมัครชั้นตรีได้' };
  ...
  const render = () => {
    ...
    $('#student-body').innerHTML = students.length ? students.map((row, index) => {
      const complete = rowComplete(row); const partial = Boolean(normalizeDigits(row.citizen) || row.exam_status || row.special_needs || row.notes); const recommendation = recommendationFor(row); const hasManualPrevious = Boolean(String(row.previous || '').trim() && String(row.previous_certificate_year || '').trim()); const canSelectLevel = Boolean(recommendation.level || row.application_level || hasManualPrevious); const canSearchPrevious = !String(row.previous || '').trim() || !['auto_matched', 'confirmed'].includes(row.match_status); const selectedApplicationLevel = row.application_level || recommendation.level || ''; const code = levelCode[selectedApplicationLevel] || 'tri';
      ...
      return `<tr class="student-row ${complete ? 'complete' : partial ? 'partial' : 'empty'} ${rowKind}" data-number="${esc(row.number)}" data-application-level="${code}">
        <td>${index + 1}</td><td>${esc(row.number)}</td><td><button type="button" class="student-name-button" data-edit-name-number="${esc(row.number)}" title="แก้ไขชื่อ–สกุล">${esc(row.name)}</button></td>
    ...
    document.querySelectorAll('[data-edit-name-number]').forEach((button) => button.addEventListener('click', () => openNameEditModal(button.dataset.editNameNumber)));
  ...
  const saveNameEdit = async (event) => {
    ...
    try {
      const response = await fetch(`${supabaseUrl}/rest/v1/rpc/activity_update_student_name_v1`, {
        method:'POST', cache:'no-store',
        headers:{ apikey:supabaseKey, Authorization:`Bearer ${token}`, 'Content-Type':'application/json' },
        body:JSON.stringify({ requested_year:'2569', requested_role:access.role, requested_access_code:access.code, requested_student_number:row.number, current_full_name:row.name, requested_full_name:name }),
      });
      const payload = await response.json().catch(() => null);
      const result = Array.isArray(payload) ? payload[0] : payload;
      ...
      closeNameEditModal();
      setStatus('แก้ไขชื่อ–สกุลเข้าฐานกลางแล้ว','#167047');
    } catch (error) {
      $('#name-edit-message').textContent = `ยังไม่บันทึก: ${error.message}`;
    }
  };
  ...
  const requestSheetSync = async (button) => {
    ...
    try {
      const response = await fetch(`${supabaseUrl}/functions/v1/sync-google-sheets`, { method:'POST', headers:{ apikey:supabaseKey, Authorization:`Bearer ${staffToken}`, 'Content-Type':'application/json' }, body:JSON.stringify({ requested_year:'2569' }), cache:'no-store' });
      const payload = await response.json().catch(() => null);
      if (!response.ok) throw new Error(payload?.message || payload?.error || `Supabase HTTP ${response.status}`);
      setStatus(`ซิงก์ชีทสำเร็จ · ตรี ${payload?.counts?.['ตรี'] ?? 0} · โท ${payload?.counts?.['โท'] ?? 0} · เอก ${payload?.counts?.['เอก'] ?? 0}`, '#167047');
      window.setTimeout(() => { void syncStatus().catch(() => {}); }, 500);
    } catch (error) {
  ...
  const organizeControlBar = () => {
    const header = document.querySelector('.control-bar');
    if (!header || header.dataset.organized === 'true') return;
    header.dataset.organized = 'true';
    const brand = header.querySelector('.brand');
    const templates = header.querySelector('.template-links');
    const printButton = $('#print-button');
    const printAllButton = $('#print-all-button');
    const syncButton = $('#sync-now-button');
    const fontButton = header.querySelector('.font-button');
  ...
  $('#roster-editor-button').addEventListener('click', () => openRosterModal()); $('#roster-form').addEventListener('submit', saveRosterEdit); document.querySelectorAll('[data-close-roster-modal]').forEach((element) => element.addEventListener('click', closeRosterModal)); $('#name-edit-form').addEventListener('submit', saveNameEdit); document.querySelectorAll('[data-close-name-modal]').forEach((element) => element.addEventListener('click', closeNameEditModal)); $('#edit-previous').addEventListener('input', syncRosterExamOptions); $('#edit-previous-year').addEventListener('input', syncRosterExamOptions); $('#edit-application-level').addEventListener('change', syncRosterExamOptions); $('#edit-exam').addEventListener('change', () => syncRosterExamChecks('exam')); $('#edit-no-exam').addEventListener('change', () => syncRosterExamChecks('no-exam')); $('#edit-special').addEventListener('change', () => syncRosterExamChecks('special'));
